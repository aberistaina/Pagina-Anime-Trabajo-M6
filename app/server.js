...
app.get("/animecards",async (req, res) => {
    let { busqueda } = req.query
    let data = await fn.leerAnime()
    let animes = data.animes
...
app.post("/api/v1/animes", async (req, res) =>{
    try {
        let data = await fn.leerAnime()
        let {nombre, genero, year, autor, imagen, descripcion} = req.body
        ...
        if(indice !== -1){
            ...
            if(!isNaN(yearNumero)){
            let animeNuevo = new Anime({nombre, genero, year, autor, imagen, descripcion})
...
app.put("/api/v1/animes/:id", async (req, res)=> {
    try {
        ...
        let yearNumero = Number(year)
            if(!isNaN(yearNumero)){
                let animeEditar = new Anime({nombre, genero, year, autor, imagen, descripcion})
